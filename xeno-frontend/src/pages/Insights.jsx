...
const Insights = () => {
  ...
  const stats = [
    { name: 'Total Customers', value: insights?.totalCustomers || 0, icon: Users, color: 'bg-blue-500' },
    { name: 'Total Orders', value: insights?.totalOrders || 0, icon: ShoppingCart, color: 'bg-green-500' },
    { name: 'Total Revenue', value: `$${Number(insights?.totalRevenue || 0).toLocaleString()}`, icon: DollarSign, color: 'bg-yellow-500' },
  ];

  // Placeholder trend data (frontend only)
  const revenueData = [
            ...
            <div className="flow-root">
              <ul className="-my-5 divide-y divide-gray-200">
                {insights.topCustomers.map((customer, index) => {
                  const name = `${customer.first_name || ''} ${customer.last_name || ''}`.trim() || customer.email || 'Unknown Customer';
                  return (
